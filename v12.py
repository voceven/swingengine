...
''')
    ...
    # Bootstrap engine/report.py
    report_path = os.path.join(engine_dir, 'report.py')
    if not _is_valid_py(report_path):
        with open(report_path, 'w') as f:
            f.write('''# -*- coding: utf-8 -*-
"""Report Generation Functions for Swing Trading Engine v12"""
import pandas as pd
...
def print_alpha_leaderboard(stocks_df, dual_ranking_config, enable_validation=False):
    ...
    else: alpha_picks = pd.DataFrame()
    display_cols = [c for c in ['ticker', 'alpha_score', 'quality', 'has_bull_flag', 'nn_score', 'position_pct', 'risk_tier'] if c in stocks_df.columns]
    ...
    else: print("  [!] No candidates met Alpha Momentum criteria (>= 75)")
    return alpha_picks
...
def print_phoenix_leaderboard(stocks_df, dual_ranking_config, enable_validation=False):
    ...
    else: phoenix_picks = pd.DataFrame()
    display_cols = [c for c in ['ticker', 'phoenix_score', 'solidity_score', 'is_phoenix', 'quality', 'dp_support', 'position_pct'] if c in stocks_df.columns]
    if not phoenix_picks.empty:
        ...
        for _, row in phoenix_picks.head(5).iterrows():
            expl = str(row.get('explanation', 'N/A'))[:97] + "..." if len(str(row.get('explanation', ''))) > 100 else row.get('explanation', 'N/A')
            print(f"  {row.get('ticker', 'N/A')} [P:{row.get('phoenix_score', 0):.1f} S:{row.get('solidity_score', 0):.2f}]: {expl}")
    ...
    else: print("  [!] No candidates met Phoenix Reversal criteria (>= 60)")
...
def print_momentum_strategy(stocks_df):
    ...
    print("="*80)
    display_cols = [c for c in ['ticker', 'trend_score', 'quality', 'has_bull_flag', 'nn_score', 'position_pct', 'shares', 'risk_tier'] if c in stocks_df.columns]
    if not trend_picks.empty:
        ...
        for _, row in trend_picks.head(5).iterrows():
            pct, shares = row.get('position_pct', 0), int(row.get('shares', 0))
            print(f"  {row['ticker']}: {pct:.1f}% (${pct*1000:.0f} → {shares} shares @ ${row.get('current_price', 0):.2f}) | {row.get('risk_tier', 'N/A')}")
        print("\\n  --- SIGNAL EXPLANATIONS ---")
        for _, row in trend_picks.head(5).iterrows():
            expl = str(row.get('explanation', 'N/A'))[:117] + "..." if len(str(row.get('explanation', ''))) > 120 else row.get('explanation', 'N/A')
            print(f"  {row['ticker']}: {expl}")
    else: print("  [!] No candidates met strict criteria (>80).")
    return trend_picks

def print_ambush_strategy(stocks_df):
    ambush_picks = stocks_df[(stocks_df['ambush_score'] > 75) & ((stocks_df['is_reversal_setup'] == True) | (stocks_df['sector_status'] == 'Lagging Sector'))].sort_values('ambush_score', ascending=False)
    print("\\n" + "="*80)
    ...
    print("="*80)
    display_cols = [c for c in ['ticker', 'ambush_score', 'quality', 'is_reversal_setup', 'dp_support', 'gex_support', 'position_pct', 'shares'] if c in stocks_df.columns]
    if not ambush_picks.empty:
        ...
        for _, row in ambush_picks.head(5).iterrows():
            expl = str(row.get('explanation', 'N/A'))[:117] + "..." if len(str(row.get('explanation', ''))) > 120 else row.get('explanation', 'N/A')
            print(f"  {row['ticker']}: {expl}")
    else: print("  [!] No reversal candidates found.")
    return ambush_picks
...
def print_bull_flag_strategy(stocks_df):
    ...
    if not flag_picks.empty:
        ...
        print("="*80)
        display_cols = [c for c in ['ticker', 'trend_score', 'quality', 'nn_score', 'position_pct', 'shares', 'stop_loss', 'take_profit'] if c in stocks_df.columns]
...
def print_etf_strategy(etfs_df):
    if not etfs_df.empty:
        ...
        print("="*80)
        etf_display = [c for c in ['ticker', 'trend_score_val', 'sector_status', 'net_gamma', 'nn_score', 'stop_loss', 'take_profit'] if c in etfs_df.columns]
...
def print_phoenix_strategy(stocks_df):
    ...
    if not phoenix_df.empty:
        phoenix_display = [c for c in ['ticker', 'phoenix_score', 'solidity_score', 'quality', 'nn_score', 'position_pct', 'shares', 'stop_loss', 'take_profit'] if c in phoenix_df.columns]
        ...
        for _, row in phoenix_sorted.iterrows():
            print(f"  {row['ticker']}: {str(row.get('explanation', 'No explanation'))[:150]}...")
    else: print("  [!] No phoenix reversal candidates found.")
    return phoenix_df

def print_summary(stocks_df, etfs_df, alpha_picks, phoenix_picks, trend_picks, ambush_picks, flag_picks, phoenix_df, elapsed_time, device_name):
...
def generate_full_report(stocks_df, etfs_df, engine, dual_ranking_config, enable_validation, elapsed_time, device_name):
    ...
    msg = print_summary(stocks_df, etfs_df, alpha_picks, phoenix_picks, trend_picks, ambush_picks, flag_picks, phoenix_df, elapsed_time, device_name)
